// mpi dijkstra with performance logging

// Compile:
// mpic++ -std=c++17 -O2 mpi_dijkstra.cpp -o mpi_dijkstra
// Run:
// mpirun -np 4 ./mpi_dijkstra
...
// Dijkstra local (unused here, placeholder)
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        g = load_graph("Dataset/com-dblp.ungraph.txt", total_nodes);
    ...
    if (rank==0) {
        ...
        std::ofstream perf("perf_mpi.csv", std::ios::app);
        if (perf.is_open()) {
            perf << size << "," << 1 << "," << elapsed << "\n";
            perf.close();
        } else {
            cerr << "Warning: could not open perf_mpi.csv for writing\n";
        }
    }
