// openmp_dijkstra_with_metrics.cpp with performance logging

// Compile: 
// g++ -O3 -std=c++17 openmp_dijkstra_with_metrics.cpp -fopenmp -lmetis -o openmp
...
int main() {
    ...
    // 1) Load & remap graph
    const string infile = "Dataset/com-dblp.ungraph.txt";
    ifstream fin(infile);
    if (!fin) { 
    ...
    // Log performance for scalability evaluation
    ofstream perf("perf_mpi_omp.csv", ios::app);
    if (perf.is_open()) {
        int threads = omp_get_max_threads();
        perf << 1 << "," << threads << "," << time_total << "\n";
        perf.close();
    } else {
        cerr << "Warning: could not open perf_mpi_omp.csv for writing\n";
    }
